...
function allowed(profile: DashboardProfile, route: V3ReadRoute, uid: string): boolean {
  if (profile.activeRole === "student") {
    if (route.path === "/v3/shared/mode" || route.path === "/v3/shared/tests" || route.path === "/v3/shared-web/resolve/{token}" || route.path === "/v3/shared-web/enter/{token}" || route.path.startsWith("/v3/test/")) return true;
  ...
  return route.path.startsWith("/v3/paperdesk/");
}
