...
interface RestoreClassroomRouteDependencies extends AdminRouteDependencies {
...
export function createRestoreClassroomRouteHandler(dependencies: RestoreClassroomRouteDependencies) {
  ...
  return (request: Request, routeContext: ClassroomRouteContext) => serveHttp(request, async ({ correlationId }) => {
    ...
    const classroom = await dependencies.classrooms.restore(principal, classroomId, {
...
export async function POST(request: Request, context: ClassroomRouteContext): Promise<Response> {
  try {
    ...
    return createRestoreClassroomRouteHandler(dependencies)(request, context);
  } catch (error) {
