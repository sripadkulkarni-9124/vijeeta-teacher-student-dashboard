...
describe("V3 BFF runtime configuration", () => {
  ...
  it("rejects fixture mode or local persistence in production", () => {
    const base = { NODE_ENV: "production", VIJEETA_V3_BASE_URL: "https://v3.example.test", VIJEETA_FIRESTORE_DATABASE_ID: "vijeeta-dashboard", VIJEETA_FIREBASE_PROJECT_ID: "neetcompanion-50b1f" };
  ...
  it("returns bounded production config", () => {
    const config = loadRuntimeConfig({ NODE_ENV: "production", VIJEETA_V3_BASE_URL: "https://v3.example.test/", VIJEETA_FIRESTORE_DATABASE_ID: "vijeeta-dashboard", VIJEETA_FIREBASE_PROJECT_ID: "neetcompanion-50b1f", VIJEETA_V3_TIMEOUT_MS: "4000", VIJEETA_BUILD_ID: "b1" });
    expect(config.baseUrl.toString()).toBe("https://v3.example.test/");
  ...
  it("rejects the default or an unexpected Firestore database in production", () => {
    const base = { NODE_ENV: "production", VIJEETA_V3_BASE_URL: "https://v3.example.test", VIJEETA_FIREBASE_PROJECT_ID: "neetcompanion-50b1f" };
  ...
  it("requires the explicitly approved Firebase project in production", () => {
    const base = { NODE_ENV: "production", VIJEETA_V3_BASE_URL: "https://v3.example.test", VIJEETA_FIRESTORE_DATABASE_ID: "vijeeta-dashboard" };
